								  ...
								  pmdEDUCB *cb)
{
	...
	*disconnect                 = false;
	...
	try
	{
		...
		{
			PD_LOG(PDEVENT, "Recieve disconnect msg");
			*disconnect = true;
		}
		else
		{
	...
	{
		PD_LOG(PDERROR,
			   "Error happened during performing opreation: %s",
			   e.what());
		probe = 70;
		rc = EDB_INVALIDARG;
		goto error;
	...
	{
		PD_LOG(PDERROR,
			   "Failed to perform opreation, rc = %d", rc);
		goto error;
	}
done:
	// build reply
...
int pmdAgentEntryPoint(pmdEDUCB *cb, void *arg)
{
	...
	while(!disconnect)
	{
		...
		PD_LOG(PDDEBUG, 
				"Recieve packet size = %d", packetLength);
		...
		// check if current recieve buffer is large enough for the package
		if(receiveBufferSize < packetLength+1)
		{
			PD_LOG(PDDEBUG, 
					"Recieve buffer size is too small: %d vs %d, increasing",
...
error:
	...
	case EDB_INVALIDARG:
		PD_LOG(PDSEVERE,
				"Invalid atgument recieved by agent, probe %d", probe);
		break;
	case EDB_OOM:
		PD_LOG(PDSEVERE,
