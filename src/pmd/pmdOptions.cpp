...
int pmdOptions::importVM(const po::variables_map &vm, bool isDefault)
{
	...
	const char *p = NULL;
	//conf path
	if(vm.count(PMD_OPTIONS_CONFPATH))
	{
		p = vm[PMD_OPTIONS_CONFPATH].as<string>().c_str();
	...
	// log path
	if(vm.count(PMD_OPTIONS_LOGPATH))
	{
		p = vm[PMD_OPTIONS_LOGPATH].as<string>().c_str();
	...
	// db file path
	if(vm.count(PMD_OPTIONS_DBPATH))
	{
		p = vm[PMD_OPTIONS_DBPATH].as<string>().c_str();
	...
	// svcname
	if(vm.count(PMD_OPTIONS_SVCNAME))
	{
		p = vm[PMD_OPTIONS_SVCNAME].as<string>().c_str();
	...
	// maxpool
	if(vm.count(PMD_OPTIONS_MAXPOOL))
	{
		_maxPool = vm[PMD_OPTIONS_MAXPOOL].as<unsigned int>();
	}
	else if(isDefault)
	{
		_maxPool = NUMPOOL;
...
int pmdOptions::init(int argc, char **argv)
{
	...
	if(vm.count(PMD_OPTIONS_CONFPATH))
	{
		rc = readConfigureFile(vm[PMD_OPTIONS_CONFPATH].as<string>().c_str(), all, vm2);
	}
	if(rc)
	{
